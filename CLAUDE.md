...
1. 工程 ③ の編集プロキシ（`ZaimClient` に更新系メソッドを足すところから）。
   ...
   **Zaim の PUT は部分更新。ただし `amount` だけが例外で、省略すると 0 になる**
   ...
   `genre_id` / `from_account_id` / `to_account_id` / `name` / `place` / `comment` は
   省略しても保持され、空文字を明示的に送れば消せる。`amount` を落としたときも
   HTTP は 200 が返るため、呼び出し側からは成功に見える。**`ZaimClient` の更新系
   メソッドでは `amount` を必須引数にして型で強制する。** 「メモだけ直す」という
   一番軽い編集がこの地雷を踏む形なので、任意にして呼び出し側の裁量に任せない。
   なお `income` の作成 API は `name` を受け付けないが、更新 API は受け付ける。

   検証していないのは transfer と、画面に出していない列（`receipt_id`、`place_uid`、
   `active`、`currency_code`）の扱い。連携由来の明細を編集対象にする前に確かめる。
