...
Zaim 本体のフィルタが弱く、**金融機関連携で自動的に入ってくる細かな履歴に、
振替や大きな入出金が埋もれる**。「1,000 円未満を消す」「振替を除く」といった
絞り込みを重ねたいのに、本体の画面ではそれができない。

実データで言うと、全 4,370 件のうち読みたいものは 1,881 件しかない（2026-08 時点。
件数は同期のたびに増える）。
...
| 全件 | 4,370 |
| 振替を除く | 3,879 |
| 未来の予定（繰り返し登録の家賃）を隠す | 3,839 |
| 1,000 円未満のノイズを除く | 1,881 |
...
> [!NOTE]
> 同じ構成を圧縮した図が [`AGENTS.md`](AGENTS.md) の冒頭にもある。あちらは
> エージェントが毎セッション読み込む前提で置いてあるので、リンクに畳まず
> 両方に持っている。**構成が変わったら 2 つとも直す。**

## 現在地

| 工程 | 状態 |
|---|---|
| ① 同期基盤（Zaim → D1） | 完了。全件 4,370 件をミラー済み。実測 22 秒で差し替わる |
...
判断を覆す提案をする前に、対応する ADR を読むこと。理由と当時の測定値は
そこにしか残っていない。
