 ...
 * 金額として受け付ける上限。
 *
 * 上限が無くても、`coercedInt()` の `v.safeInteger()` が安全な整数（2^53-1 =
 * 9,007,199,254,740,991）を超えた値は 400 で弾く。`<input type="number">` は
 * 桁数を制限しないので、17 桁を入れれば到達する。ただしそれだけだと 400 の理由が
 * 伝わらないため、`MAX_AMOUNT` で「桁が大きすぎる」と分かる上限を明示している
